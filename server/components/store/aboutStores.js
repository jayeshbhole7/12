...
exports.getAllStores = async (req, res) => {
  ...
  let query = 'SELECT s.id, s.name, s.email, s.address, COALESCE(AVG(r.rating), 0) as average_rating FROM stores s LEFT JOIN ratings r ON s.id = r.store_id';
  let conditions = [];
  let values = [];
  let i = 1;
...
exports.addStore = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    if (owner_id) {
      const ownerRes = await pool.query('SELECT id, role FROM users WHERE id = $1', [owner_id]);
